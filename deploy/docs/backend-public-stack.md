...
`deploy-backend-init`은 시작하자마자 `make reset-backend`를 실행해 frontend를 제외한 compose 컨테이너, compose 볼륨, 로컬 backend 이미지를 지우고 `deploy/.local/onboarding-service-artifacts/onboarding_category_tower`도 비운 뒤 다시 배포한다.
